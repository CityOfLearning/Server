...
public class GetScheduledPrograms extends Thread {
	...
	public GetScheduledPrograms(int ord_id, int program_id, String secret, String key) {
		orgId = "" + ord_id;
	...
	public void run() {
		try {
			...
			HttpGet getReq = new HttpGet(String.format(
					"http://chicago.col-engine.com/partner_api/v1/orgs/%s/programs/%s/scheduled_programs.json?jwt=%s",
					orgId, programId, token.serializeAndSign()));
